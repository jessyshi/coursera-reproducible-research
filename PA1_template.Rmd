...
This report contains the codes and the results for the submission of the first project in the Coursera course: **Reproducible Research**
...
```
Notice that the date variable is a character type, convert this to date format
...
Next, a histogram of the total number of steps taken each day is plotted.
```{r}
...
Lastly, the mean and median of the total number of steps taken per day are calculated
...
print(median_steps)
```
The mean and median of the total number of steps takern per day are 10766 and 10765 respectively.


## III.Average Daily Activity Pattern

Firstly, a time series plot of the 5-minute interval (x-axis) and the average number of steps taken is created, averaged across all days (y-axis).
...
When the data was first loaded, it is clearly seed that there are some missing values (coded as NA).

Total number of missing values in the dataset is calculated:
```{r}
sum(is.na(activity))
```
The presence of missing days may introduce bias into some calculations or summaries of the data. In this part of the project, the missing values are filled with the mean of all the other values. This new dataset is called new_activity.
...
Mean and median of total number of steps taken per day for the new dataset are calculated.
...
```
After imputation of missing data, the median and mean values become the same.


Next, a histogram of the total number of steps taken each day using the new dataset is plotted.
...
In this part of the project, the dataset with the filled-in missing values are used.
...
A new variable is assigned, with 2 levels - "weekday" and "weekend" indicating whether a given date is a weekday or weekend day.

```{r}
new_activity$day<-ifelse(weekdays(new_activity$date) %in% c("Saturday","Sunday"),"weekend",'weekday')
```

Lastly, a panel plot is created, containing a time series plot of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all weekday days or weekend days (y-axis). 
```{r}
